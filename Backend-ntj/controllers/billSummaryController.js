...
const buildBillPayload = (body = {}) => {
    ...
    return {
        ...
        isNilBalance: Boolean(body.isNilBalance ?? body.summary?.isNilBalance),
        isNilFT: Boolean(body.isNilFT ?? body.summary?.isNilFT),
        ftRate: toNumber(body.ftRate ?? body.summary?.ftRate),
        resultFtRate: toNumber(body.resultFtRate ?? body.summary?.resultFtRate ?? body.ftRate ?? body.summary?.ftRate),
        resultValue: toNumber(body.resultValue ?? body.summary?.resultValue ?? body.nilBalanceAmount ?? body.summary?.nilBalanceAmount),
        baseBalance: toNumber(body.baseBalance ?? body.summary?.baseBalance ?? body.summaryData?.baseBalance),
        returnAmount: toNumber(body.returnAmount ?? body.summary?.returnAmount),
        returnValue: toNumber(body.returnValue ?? body.summary?.returnValue),
        finalAmount: toNumber(body.finalAmount ?? body.summary?.finalAmount ?? body.finalValue ?? body.summary?.finalValue),
        finalValue: toNumber(body.finalValue ?? body.summary?.finalValue ?? body.nilFtValue ?? body.summary?.nilFtValue),
        summaryData: body.summaryData || body.previewSnapshot?.summaryData || body.summary?.summaryData || null,
