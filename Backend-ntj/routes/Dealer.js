...
const dealerPublicProjection = "-image -receiptImage -proofImage -receiptImageShowInBill";

const stripDealerImageFields = (payload) => {
  const clean = { ...(payload || {}) };
  delete clean.image;
  delete clean.receiptImage;
  delete clean.proofImage;
  delete clean.receiptImageShowInBill;
  return clean;
};
...
router.post("/", async (req, res) => {
  try {
    ...
    const newDealer = new Dealer(stripDealerImageFields({
      customerName,
      phoneNumber,
      gstin,
      address,
    ...
    const { image, receiptImage, proofImage, receiptImageShowInBill, ...safeDealer } =
      savedDealer.toObject();
    res.status(201).json(safeDealer);
  } catch (error) {
    console.error(error);
...
router.get("/:id", async (req, res) => {
  try {
    const dealer = await Dealer.findById(req.params.id).select(dealerPublicProjection);
    if (!dealer) {
...
router.put("/:id", async (req, res) => {
  try {
    const updatedDealer = await Dealer.findByIdAndUpdate(
      req.params.id,
      {
        ...stripDealerImageFields(req.body),
      },
