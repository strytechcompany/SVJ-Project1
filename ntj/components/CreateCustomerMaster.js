...
export default function CreateCustomerMaster({ navigation, route }) {
  ...
  const saveCustomer = async () => {
    ...
    const nameExists = customers.some(
      (c) =>
        String(c?.customerName || c?.name || "")
          .trim()
          .toLowerCase() === customerName.trim().toLowerCase()
    );
    if (nameExists) {
      Alert.alert(
        "Duplicate Name ❌",
    ...
    const newCustomer = {
      ...
      companyName: normalizedShopName,
      gstin: "",
    ...
    try {
      ...
      if (response.ok && data) {
        ...
        // Go back and hint the previous screen to refresh its list immediately.
        const refreshToken = Date.now();
        const state = navigation.getState?.();
        const prevRoute =
          state?.routes && Number.isFinite(state.index) ? state.routes[state.index - 1] : null;
        ...
        console.warn("CreateCustomerMaster save failed:", response.status, data);
  ...
  // Render input helper — required=true adds *, optional fields have no asterisk
  const renderInput = (label, value, setter, keyboard = "default", required = false) => (
    ...
    <View style={{ flex: 1, backgroundColor: "#fff" }}>
      ...
      >
        ...
        >
          ...
          {renderInput(customerType === "Dealer" ? "Dealer Name" : "Customer Name", customerName, setCustomerName, "default", true)}
          {renderInput("Phone Number", customerNumber, setCustomerNumber, "phone-pad", true)}
