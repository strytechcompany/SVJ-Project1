...
export default function PaymentsScreen() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [billRes, paymentRes, dealerRes, stockRes] = await Promise.all([
  ...
  const stockMap = useMemo(() => {
    ...
    stockRecords.forEach(s => {
      map[s.itemName.toLowerCase()] = parseFloat(s.grossWeight || 0);
    });
    return map;
  }, [stockRecords]);
  ...
  const handleTransfer = async () => {
    ...
    try {
      ...
      const stockRecord = currentStocks.find(s =>
        s.itemName.toLowerCase() === catalogItem.itemName.toLowerCase()
      );
      ...
      if ((stockRecord.grossWeight || 0) < weightToSub) {
        Alert.alert("Insufficient Stock", `Available stock for ${stockRecord.itemName} is ${stockRecord.grossWeight}g.`);
        return;
      }

      // ---- STEP 1: Update StockMaster ----
      const updatedWeight = Math.max(0, (stockRecord.grossWeight || 0) - weightToSub);
      const updatedNetWeight = Math.max(0, (stockRecord.netWeight || 0) - weightToSub);
      const newPure = (stockRecord.pure || 0) * (updatedWeight / (stockRecord.grossWeight || 1));
      ...
      const transferRecord = {
        ...
        selectedItems: [catalogItem.itemName],
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "#FDFDFD" }}>
      ...
      <Icon name="chevron-left" size={28} color="#3D2800" />
      </TouchableOpacity>
      }
      right={
      <TouchableOpacity style={styles.headerIconBtn} onPress={fetchData} disabled={loading}>
      <Icon name="refresh" size={24} color="#3D2800" />
      </TouchableOpacity>
      }
      />
        ...
        <ScrollView contentContainerStyle={styles.scrollContainer} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.profileCard}>
            <View style={styles.profileCircle}>
              <Icon name="account" size={30} color="#3D2800" />
            </View>
          ...
          <View style={styles.summaryGrid}>
            <View style={[styles.summaryBox, { backgroundColor: "#FFF8E1" }]}>
              <View style={styles.summaryCircle}><Icon name="arrow-down-bold" size={20} color="#B8860B" /></View>
              <View>
                <Text style={styles.summaryLabel}>TOTAL IN</Text>
                <Text style={[styles.summaryValue, { color: "#B8860B" }]}>{grandTotalIn} g</Text>
              </View>
            </View>
          ...
          <View style={styles.card}>
            ...
            {loading && items.length === 0 ? (
              ...
              items.map((item, index) => {
                const nameKey = (item.itemName || "").toLowerCase();
                ...
                return (
                  ...
                  >
                    ...
                    <Text style={styles.itemNameCell} numberOfLines={1}>{item.itemName}</Text>
                    ...
                    <View style={styles.checkCell}>
                      <Icon
                        ...
                        size={22}
                        color={isSelected ? "#3D2800" : "#E0E0E0"}
                      />
                    </View>
                  </TouchableOpacity>
                );
          ...
          <View style={[styles.card, styles.controlCard]}>
            ...
            <View style={styles.inputGroup}>
              ...
              <View style={styles.weightInputWrapper}>
                ...
                />
                <Text style={{ marginRight: 15, fontWeight: "bold", color: "#3D2800" }}>GRAMS</Text>
              </View>
            </View>
            <View style={styles.inputGroup}>
              ...
              <TouchableOpacity style={styles.dealerSelect} onPress={() => setModalVisible(true)}>
                ...
                <Icon name="chevron-down" size={22} color="#3D2800" />
              </TouchableOpacity>
            </View>
            <TouchableOpacity
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              ...
              <TouchableOpacity onPress={() => setModalVisible(false)}><Icon name="close-circle" size={32} color="#3D2800" /></TouchableOpacity>
            </View>
            <View style={styles.modalSearch}>
              <Icon name="magnify" size={24} color="#3D2800" />
...
const styles = StyleSheet.create({
  ...
  headerTitle: { fontSize: 18, fontWeight: "900", color: "#3D2800", letterSpacing: 0.5 },
  headerBatch: { fontSize: 10, color: "#666", fontWeight: "bold", textTransform: "uppercase" },
  scrollContainer: { padding: 16, paddingBottom: 60 },
  datePanel: { backgroundColor: "#3D2800", borderRadius: 20, padding: 18, marginBottom: 20, flexDirection: "row", alignItems: "center", elevation: 6 },
  ...
  inCell: { flex: 1, textAlign: "center", fontSize: 13, color: "#B8860B", fontWeight: "900" },
  ...
  controlTitle: { fontSize: 16, fontWeight: "900", color: "#3D2800", marginBottom: 25, textAlign: "center" },
  inputGroup: { marginBottom: 20 },
  inputLabel: { fontSize: 12, fontWeight: "800", color: "#555", marginBottom: 10, textTransform: "uppercase" },
  weightInputWrapper: { flexDirection: "row", alignItems: "center", backgroundColor: "#F9FAF1", borderRadius: 14, borderWidth: 1.5, borderColor: "#FFF8E1" },
  weightInput: { flex: 1, height: 52, fontSize: 16, paddingHorizontal: 12, color: "#3D2800", fontWeight: "bold" },
  dealerSelect: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", backgroundColor: "#F9FAF1", borderRadius: 14, borderWidth: 1.5, borderColor: "#FFF8E1", padding: 15 },
  dealerSelectText: { fontSize: 15, fontWeight: "700", color: "#3D2800" },
  actionBtn: { backgroundColor: "#3D2800", flexDirection: "row", height: 60, borderRadius: 16, alignItems: "center", justifyContent: "center", marginTop: 15, elevation: 8 },
  ...
  modalTitle: { fontSize: 20, fontWeight: "900", color: "#3D2800" },
  ...
  itemIcon: { width: 44, height: 44, borderRadius: 14, backgroundColor: "#FFF8E1", alignItems: "center", justifyContent: "center", marginRight: 15 },
  itemInitial: { color: "#3D2800", fontWeight: "900", fontSize: 18 },
  ...
  profileName: { fontSize: 20, fontWeight: "900", color: "#3D2800" },
