...
export default function CreateTransaction({ navigation }) {
  ...
  const saveCompleteTransaction = async () => {
    ...
    try {
      ...
      let updateEndpoint;
      if (selectedCustomer.customerType === "B2C") {
        updateEndpoint = `${base_url}/customersB2C/${selectedCustomer.id}`;
      } else if (selectedCustomer.customerType === "Dealer") {
        updateEndpoint = `${base_url}/customersDealer/${selectedCustomer.id}`;
      } else {
        updateEndpoint = `${base_url}/customers/${selectedCustomer.id}`;
      }
  ...
  return (
    ...
    >
      <View
        style={{ paddingTop: 40, paddingHorizontal: 16, paddingBottom: 300 }}
      >
        {/* HEADER */}
                        ...
                        >
                          <View>
                            ...
                            <Text style={styles.balanceText}>
                              OB: {currentBalance.toFixed(3)}g{" "}
                              {advanceBalance > 0
                                ? `| AB: ${advanceBalance.toFixed(3)}g`
                                : ""}
                            </Text>
                          </View>
                        </TouchableOpacity>
