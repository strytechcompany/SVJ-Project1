...
export default function CreateTransaction({ navigation }) {
  ...
  // Pure calculation for Issue Table entries (New B2B Margin Formula)
  ...
  const addIssueItem = async () => {
    ...
    const purity = calcIssuePure(weight, plusValue);
          ...
          prev.map((item) =>
            item.id === editingIssueId
              ? {
                  ...
                  weight: issueWeight,
                  buyingCost: parseNum(buyingCost),
                  sellingCost: parseNum(sellingCost),
                  plus: plusValue,
                  purity,
  ...
  const editIssueItem = (row) => {
    ...
    setBuyingCost(String(row.buyingCost ?? ""));
    setSellingCost(String(row.sellingCost ?? ""));
    setPlusValue(Number(row.plus ?? 0));
    ...
    try {
      ...
      const billSummaryData = {
        ...
        issueItems: issueItems.map((item) => ({
          ...
          m: fmt(0), // Stone removed in new B2B
          net: fmt(item.weight),
          calc: fmt(item.plus),
      ...
      >
        ...
        {selectedCustomer && (
          <View style={[styles.card, styles.issueEntryCard]}>
            ...
            <View style={styles.row}>
              <View style={styles.inputBox}>
                <Text style={styles.subLabel}>Buying Cost (%)</Text>
                <TextInput
                  style={[styles.input, { backgroundColor: '#F0F0F0' }]}
                  value={buyingCost}
                  editable={false}
              ...
              <View style={styles.inputBox}>
                <Text style={styles.subLabel}>Selling Cost (%)</Text>
                <TextInput
                  style={styles.input}
                  value={sellingCost}
                  onChangeText={(val) => {
                    setSellingCost(val);
                    const b = parseNum(buyingCost);
              ...
              <View style={styles.inputBox}>
                ...
                <View style={[styles.purityBox, { backgroundColor: '#FFF3E0' }]}>
                  <Text style={[styles.purityText, { color: '#E65100' }]}>
                    {fmt(calcIssuePure(weight || 0, plusValue))} g
                  </Text>
                </View>
              </View>
            </View>
            ...
            <ScrollView horizontal={true} showsHorizontalScrollIndicator={true}>
              <View style={styles.productTable}>
                <View style={styles.productTableHeader}>
                  <Text style={[styles.productHeaderCell, { width: 100 }]}>Action</Text>
                  <Text style={[styles.productHeaderCell, { width: 40 }]}>#</Text>
                  <Text style={[styles.productHeaderCell, { width: 140 }]}>Item</Text>
                  <Text style={[styles.productHeaderCell, { width: 100 }]}>Weight (g)</Text>
                  <Text style={[styles.productHeaderCell, { width: 100 }]}>Buy (%)</Text>
                  <Text style={[styles.productHeaderCell, { width: 100 }]}>Sell (%)</Text>
                  <Text style={[styles.productHeaderCell, { width: 80 }]}>Plus (%)</Text>
                  <Text style={[styles.productHeaderCell, { width: 100 }]}>Pure (g)</Text>
                </View>
                ...
                {issueItems.map((row, idx) => (
                  <View key={row.id} style={styles.productTableRow}>
                    ...
                    <Text style={[styles.productCell, { width: 40 }]}>{idx + 1}</Text>
                    <Text style={[styles.productCell, { width: 140 }]}>{row.item}</Text>
                    <Text style={[styles.productCell, { width: 100 }]}>{fmt(row.weight)}</Text>
                    <Text style={[styles.productCell, { width: 100 }]}>{Number(row.buyingCost || 0).toFixed(2)}</Text>
                    <Text style={[styles.productCell, { width: 100 }]}>{Number(row.sellingCost || 0).toFixed(2)}</Text>
                    <Text style={[styles.productCell, { width: 80, color: '#B8860B', fontWeight: 'bold' }]}>
                      {Number(row.plus || 0).toFixed(2)}
                    </Text>
                  ...
                  <View key={row.id} style={styles.productTableRow}>
                    ...
                    >
                      <TouchableOpacity onPress={() => editReceiptItem(row)}>
                        <Text style={[styles.actionText, { color: "#B8860B" }]}>
                          Edit
                        </Text>
                      </TouchableOpacity>
