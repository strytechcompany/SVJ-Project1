...
const CustomerCard = ({
  ...
  const imageUri = normalizeImageUri(
    item.lastTransaction?.receiptImage ||
    item.lastTransaction?.image ||
    item.lastTransaction?.proofImage ||
    item.receiptImage ||
    item.latestTransactionImage ||
    item.image ||
    item.proofImage ||
    getRowImageRaw(item),
    base_url,
  ...
  const handleImagePress = () => {
    if (!isDealerCard || !imageUri) return;
        ...
        <View style={styles.customerDataCard} >
          ...
          >
            ...
            <View style={styles.customerDataBottomRow}>
              ...
              <TouchableOpacity onPress={handleImagePress} disabled={!imageUri}>
                <View style={styles.customerDataRightImageWrap}>
                  {imageUri ? (
                    ...
                    <View style={styles.customerDataRightImagePlaceholder}>
                      <Icon name="image-off-outline" size={30} color="#999" />
                    </View>
                  )}
                </View>
              </TouchableOpacity>
...
export default function CustomerMasterList({ navigation, route }) {
  ...
  const fetchCustomers = async () => {
    try {
      // Keep dealer image fields in DB synchronized from latest transactions/bills.
      // Best-effort: ignore failure and continue with normal fetch.
      try {
        await fetch(`${base_url}/customersDealer/sync-images`, { method: "POST" });
      } catch (_) {}
      ...
      const getLatestDealerSnapshot = (dealer) => {
        ...
        const allImageRows = [...allTxRows, ...allBillRows].sort(
          (a, b) =>
            toTs(b.updatedAt || b.createdAt || b.date || 0) -
            toTs(a.updatedAt || a.createdAt || a.date || 0),
        );
        ...
        const matchedRows = allImageRows.filter((row) => rowBelongsToDealer(row));
        ...
        const latestWithImage = matchedRows.find((row) => !!getRowImageRaw(row)) || matchedRows[0];
        ...
        return {
          image: getRowImageRaw(latestWithImage),
      ...
      const supplierCustomers = dealerData.map((dealer) => {
        ...
        const txFallbackImage =
          readImageField(dealer?.latestTransactionImage) ||
          readImageField(dealer?.lastTransaction?.receiptImage) ||
          readImageField(dealer?.lastTransaction?.proofImage) ||
          readImageField(latestSnapshot?.image) ||
          readImageField(dealer?.lastTransaction?.image) ||
          "";
        ...
        return {
          ...
          receiptImage:
            ...
            "",
          latestTransactionImage: txFallbackImage || "",
          image:
            txFallbackImage ||
            readImageField(dealer?.latestTransactionImage) ||
            readImageField(dealer?.image) ||
