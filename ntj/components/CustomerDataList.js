...
const CustomerCard = memo(({
  ...
  const balanceInfo = useMemo(() => {
    if (advanceBalance > 0) return { color: "#B8860B", blinking: false }; // Green for Advance
    if (oldBalance > 0) {
                ...
                <View style={styles.customerDataBalanceRow}>
                  ...
                  <View>
                    ...
                    <Animated.Text
                      style={[
                        styles.customerDataBalanceValue,
                        { color: advanceBalance > 0 ? "#B8860B" : "#FF6F00" },
                      ]}
                    >
...
export default function CustomerMasterList({ navigation, route }) {
  ...
  const fetchChitCustomers = useCallback(async () => {
    setChitLoading(true);
    try {
      const res = await fetch(`${base_url}/chit`);
      if (res.ok) setChitCustomers(await res.json());
  ...
  useEffect(() => {
    if (filter === "Chit") fetchChitCustomers();
  }, [filter]);
  ...
  const renderChitItem = useCallback(({ item }) => (
    <View style={styles.chitCard}>
      <View style={styles.chitCardHeader}>
        <View style={styles.chitGroupBadge}>
          <Text style={styles.chitGroupText}>{item.groupCode}</Text>
        </View>
        <Text style={styles.chitCustomerName}>{item.customerName}</Text>
        <View style={styles.iconRow}>
      ...
      {!!item.address && (
        <Text style={styles.chitAddress}>{item.address}</Text>
      )}
      <View style={styles.chitRow}>
        <View style={styles.chitCell}>
          <Text style={styles.chitLabel}>Monthly Amt</Text>
          <Text style={styles.chitValue}>₹{Number(item.monthlyAmount || 0).toLocaleString()}</Text>
        </View>
        <View style={styles.chitDivider} />
        <View style={styles.chitCell}>
          <Text style={styles.chitLabel}>Gold Rate</Text>
          <Text style={styles.chitValue}>₹{Number(item.goldRate || 0).toLocaleString()}</Text>
        </View>
        <View style={styles.chitDivider} />
        <View style={styles.chitCell}>
          <Text style={styles.chitLabel}>Cal.Wt</Text>
          <Text style={styles.chitValue}>{Number(item.calculatedWeight || 0).toFixed(3)}g</Text>
        </View>
      </View>
      <View style={styles.chitRow}>
        <View style={styles.chitCell}>
          <Text style={styles.chitLabel}>Total.Wt Paid</Text>
          <Text style={[styles.chitValue, { color: "#B8860B" }]}>{Number(item.totalWeight || 0).toFixed(3)}g</Text>
        </View>
        <View style={styles.chitDivider} />
        <View style={styles.chitCell}>
          <Text style={styles.chitLabel}>Installments</Text>
          <Text style={[styles.chitValue, { color: "#1565C0" }]}>{item.installmentNo || 0}</Text>
        </View>
        <View style={styles.chitDivider} />
        <View style={styles.chitCell}>
          <Text style={styles.chitLabel}>Remaining</Text>
          <Text style={[styles.chitValue, { color: "#E65100" }]}>
            {Math.max(0, (item.months || 12) - (item.installmentNo || 0))} months
          </Text>
        </View>
      </View>
    </View>
  ), [handlePhonePress, handleWhatsApp]);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.filterRow}>
        {["ALL", "B2B", "B2C", "Supplier", "Chit"].map((item) => (
          <TouchableOpacity
            key={item}
      ...
      {filter === "Chit" ? (
        <FlatList
          data={chitCustomers}
          keyExtractor={(item) => String(item._id || item.groupCode)}
          ...
          ListEmptyComponent={
            chitLoading ? (
              ...
              <View style={styles.emptyWrap}>
                <Text style={styles.emptyText}>No chit customers found</Text>
              </View>
            )
          }
        />
...
const styles = StyleSheet.create({
  ...
  filterActive: { backgroundColor: "#B8860B" },
