...
export default function CreateTransaction({ navigation, route }) {
  ...
  const [b2cBarcode, setB2cBarcode] = useState("");
  const [b2cScannerVisible, setB2cScannerVisible] = useState(false);
  const [cameraPermission, requestCameraPermission] = useCameraPermissions();

  // ── NEW Issue Entry fields ───────────────────────────────────
  const [issueItemNumber, setIssueItemNumber] = useState("");
  const [issueCount, setIssueCount] = useState("1");
  const [sriCost, setSriCost] = useState("");
  ...
  const fetchStockMaster = async () => {
    try {
      ...
      const formattedStock = data.map((item) => ({
        ...
        weight: parseFloat(item.grossWeight || 0),
        netWeight: parseFloat(item.netWeight || 0),
        calculation: item.buyingTouch ? item.buyingTouch.toString() : "",
  ...
  const itemDropdownData = useMemo(() => {
    ...
    return filteredItems.map((item) => {
      const t = parseFloat(item.calculation || 0);
  ...
  }, [filteredItems, rate, weight, gstPercentage, gstEnabled]);
  ...
  const addB2CReceiptItem = async () => {
    const valWeight = parseNum(receiptWeight);
    if (valWeight <= 0) {
      Alert.alert("Error", "Please enter valid Weight");
      return;
    }
    const valLess = parseNum(receiptLess);
    const valNetWeight = valWeight - valLess;
    const valActualTouch = parseNum(receiptActualTouch);
    const valTakenTouch = parseNum(receiptTakenTouch);
    const valRate = parseNum(receiptGoldRate);
    ...
    const valPurity = Number((valNetWeight * (valTakenTouch / 100)).toFixed(3));
    const valAmount = parseNum(receiptAmount) || Number((valPurity * valRate).toFixed(2));

    const receiptEntryData = {
      customerName,
      billNo: invoiceNo || "N/A",
      type: receiptType || "Old Gold",
      weight: valWeight,
      less: valLess,
      actualTouch: valActualTouch,
      takenTouch: valTakenTouch,
      purity: valPurity,
      goldRate: valRate,
      amount: valAmount,
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Failed to save receipt entry");
      }
      ...
      const newItem = {
        ...
        name: receiptType || "Old Gold",
        netWeight: valPurity, // For backward compatibility with B2C Final Submit
      };
  ...
  const addCashEntry = async () => {
    ...
    if (parseNum(gr) <= 0) {
      Alert.alert("Invalid Rate", "Please enter a valid Gold Rate greater than 0");
      return;
    }
  ...
  const addRow = async () => {
    ...
    const purity = Number((wt * (currentPlus / 100)).toFixed(3));
    ...
    try {
      ...
      const generatedBillNo = savedItem?.invoiceNo || invoiceNo;
      ...
      console.log(
        newWeight <= 0
          ? "✅ ADD ITEM - Stock item removed from DB (fully issued)"
          : "✅ ADD ITEM - Stock weight updated in DB"
      );
      ...
      const newItem = {
        ...
        plus: currentPlus,
        purity,
        cost: parseFloat((wt * parseFloat(rate || 0)).toFixed(2)),
        goldRate: parseFloat(rate || 0),
        modifiedWeight: parseFloat(modifiedWeight || 0),
        gstEnabled,
      };
      ...
      await AsyncStorage.setItem("goldRate", rate.toString());
  ...
  const fetchStockByBarcodeB2C = async (barcodeValue) => {
    ...
    try {
      ...
      // Match against already-loaded itemList so selectedItem has the right shape
      const matched = itemList.find(
        (it) => it.itemName.toLowerCase() === stock.itemName.toLowerCase()
      ) || {
        id: stock._id,
        itemName: stock.itemName,
        itemNumber: stock.itemNumber || "",
        weight: parseFloat(stock.grossWeight || 0),
        netWeight: parseFloat(stock.netWeight || 0),
        calculation: stock.buyingTouch ? stock.buyingTouch.toString() : "",
        buyingTouch: stock.buyingTouch || 0,
        purity: stock.purity || "",
        barcode: stock.barcode || code,
      };
      setItemName(matched.itemName);
      setSelectedItem(matched);
      setIssueItemNumber(matched.itemNumber || stock.itemNumber || "");
      setSriCost((matched.buyingTouch || stock.buyingTouch || 0).toString());
      setWeight(matched.netWeight > 0 ? matched.netWeight.toString() : matched.weight.toString());
      setTouch(matched.calculation || (stock.buyingTouch ? stock.buyingTouch.toString() : ""));
      setDisplayItemName(matched.itemName);
      setModifiedWeight("");
      setShowItemDropdown(false);
      setB2cBarcode(code);
    } catch {
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      ...
      // Cost-based balance: Issue (weight×rate) + GST − Receipt amounts − Cash amounts
      const issueCostTotal = items.reduce((sum, it) => sum + (it.cost || (parseFloat(it.weight || 0) * parseFloat(it.goldRate || goldRateForBalance || 0))), 0);
      const gstTotalRupees = gstEnabled
        ? (((parseFloat(sgst) || 0) + (parseFloat(cgst) || 0)) / 100) * issueCostTotal
        : 0;
      const receiptAmountTotal = b2cReceiptItems.reduce((sum, item) => sum + parseFloat(item.amount || 0), 0);
      const cashReceivedAmount = cashTable.reduce((sum, row) => sum + parseFloat(row.rupees || 0), 0);
      const remainingBalanceRupees = issueCostTotal + gstTotalRupees - receiptAmountTotal - cashReceivedAmount;
      // Convert to grams for storage (consistent with B2B)
      const remainingBalanceGrams = goldRateForBalance > 0 ? remainingBalanceRupees / goldRateForBalance : 0;
      const newCustomerOB = remainingBalanceGrams >= 0 ? parseFloat(remainingBalanceGrams.toFixed(3)) : 0;
      const newCustomerAB = remainingBalanceGrams < 0 ? parseFloat(Math.abs(remainingBalanceGrams).toFixed(3)) : 0;

      let finalBalanceForRecord = remainingBalanceGrams;
      ...
      if (resolvedCustomer?.id) {
        finalBalanceForRecord = remainingBalanceGrams;

        const updateResponse = await fetch(`${base_url}/customersB2C/${resolvedCustomer.id}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            oldBalance: newCustomerOB.toFixed(2),
            advanceBalance: newCustomerAB.toFixed(2),
          }),
        });
      ...
      const billSummaryData = {
        ...
        currentBalance: remainingBalanceGrams,
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "#F5F7FA" }}>
        ...
        >
          ...
          {showItems && (
            <View style={styles.card}>
              <View style={styles.sectionHeader}>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                  <Icon name="account-check" size={24} color="#B8860B" />
                  <Text style={[styles.articleTitle, { color: '#B8860B' }]}>Selected Customer</Text>
                </View>
                <TouchableOpacity
          ...
          {showItems && (
            <View style={styles.card}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 15 }}>
                ...
                <View style={{ alignItems: 'flex-end' }}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 2 }}>
                    ...
                    <Text style={{ fontSize: 13, fontWeight: 'bold', color: '#B8860B' }}>
              ...
              <View style={b2cLocalStyles.barcodeRow}>
                <TextInput
                  style={[styles.input, b2cLocalStyles.barcodeInput]}
                  placeholder="Enter barcode number..."
                  placeholderTextColor="#9E9E9E"
                  value={b2cBarcode}
                  onChangeText={setB2cBarcode}
                  onSubmitEditing={() => fetchStockByBarcodeB2C(b2cBarcode)}
                  returnKeyType="search"
                  keyboardType="number-pad"
                />
                <TouchableOpacity style={b2cLocalStyles.scanBtn} onPress={openB2CScanner}>
                  <Icon name="barcode-scan" size={22} color="#fff" />
                </TouchableOpacity>
              </View>

              {/* ITEM SELECTION — display card after scan, search input when empty */}
              {selectedItem ? (
                <View style={{ backgroundColor: '#F0F8FF', borderRadius: 8, padding: 12, borderWidth: 1, borderColor: '#B8860B', marginBottom: 12 }}>
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <View style={{ flex: 1 }}>
                      <Text style={{ fontSize: 15, fontWeight: 'bold', color: '#333' }}>{selectedItem.itemName}</Text>
                      {issueItemNumber ? (
                        <Text style={{ fontSize: 12, color: '#666', marginTop: 2 }}>Item No: {issueItemNumber}</Text>
                      ) : null}
                    </View>
                    <TouchableOpacity
                      onPress={() => {
                    ...
                    >
                      <Icon name="close-circle" size={22} color="#D32F2F" />
                    </TouchableOpacity>
                  </View>
                  <View style={{ flexDirection: 'row', gap: 24, marginTop: 10 }}>
                    <View>
                      <Text style={{ fontSize: 11, color: '#888' }}>Weight</Text>
                      <Text style={{ fontSize: 14, fontWeight: 'bold', color: '#333' }}>
                        {selectedItem.netWeight > 0 ? selectedItem.netWeight : selectedItem.weight} g
                      </Text>
                    </View>
                    <View>
                      <Text style={{ fontSize: 11, color: '#888' }}>SRI Cost</Text>
                      <Text style={{ fontSize: 14, fontWeight: 'bold', color: '#333' }}>
                        {sriCost || '—'}
                      </Text>
                    </View>
                    {selectedItem.category ? (
                      <View>
                        ...
                        <Text style={{ fontSize: 14, fontWeight: 'bold', color: '#333' }}>{selectedItem.category}</Text>
                      </View>
                    ) : null}
                  </View>
                </View>
                ...
                <>
                  <View style={styles.itemAutocompleteWrap}>
                    ...
                    <TextInput
                      style={styles.input}
                      placeholder="Enter item name or scan barcode above"
                      placeholderTextColor="#9E9E9E"
                      value={itemName}
                  ...
                  {showItemDropdown && Array.isArray(filteredItems) && filteredItems.length > 0 && (
              ...
              <View style={styles.row}>
                <View style={styles.inputBox}>
                  ...
                  <View style={[styles.purityBox, { backgroundColor: '#FFF9C4', padding: 10, borderRadius: 6 }]}>
                ...
                <View style={styles.inputBox}>
                  ...
                  <View style={[styles.purityBox, { backgroundColor: '#FFF3E0', padding: 10, borderRadius: 6 }]}>
                    <Text style={{ color: '#E65100', fontWeight: 'bold' }}>
                      {fmt(Number((parseFloat(weight || 0) * ((parseNum(sriBill) - parseNum(sriCost)) / 100)).toFixed(3)))} g
                    </Text>
                  </View>
                </View>
              </View>
              ...
              {/* ADD ITEM BUTTON */}
              <TouchableOpacity style={[styles.addRowBtn, { marginTop: 20 }]} onPress={addRow}>
                <Text style={styles.addRowText}>ISSUE</Text>
              </TouchableOpacity>
              ...
              {items.length > 0 && (
                <>
                  ...
                  <ScrollView horizontal>
                    <View>
                      {/* TABLE HEADER */}
                      <View style={[styles.tableHeader, { backgroundColor: '#eee', padding: 10, borderRadius: 6, flexDirection: 'row' }]}>
                        {["Bill No", "Item Name", "Weight", "Count", "Purity", "Cost (₹)", "X"].map((h, i) => (
                          <Text key={i} style={[styles.th, { flex: 1, textAlign: 'center', fontSize: 12, fontWeight: 'bold' }]}>
                            {h}
                          </Text>
                        ))}
                      </View>
                      ...
                      {/* TABLE ROWS */}
                      {items.map((row, index) => (
                        <View key={index} style={[styles.tableRow, { flexDirection: 'row', padding: 10, borderBottomWidth: 1, borderColor: '#eee' }]}>
                          <Text style={[styles.td, { flex: 1, textAlign: 'center' }]}>{row.billNo}</Text>
                          <Text style={[styles.td, { flex: 1, textAlign: 'center' }]}>{row.displayItemName || row.itemName}</Text>
                          <Text style={[styles.td, { flex: 1, textAlign: 'center' }]}>{fmt(row.weight)}</Text>
                          <Text style={[styles.td, { flex: 1, textAlign: 'center' }]}>{row.count}</Text>
                          <Text style={[styles.td, { flex: 1, textAlign: 'center', fontWeight: 'bold' }]}>{fmt(row.purity)}</Text>
                          <Text style={[styles.td, { flex: 1, textAlign: 'center', color: '#B8860B', fontWeight: 'bold' }]}>
                            {Number(row.cost || (parseFloat(row.weight || 0) * parseFloat(row.goldRate || rate || 0))).toFixed(2)}
                          </Text>
                          <TouchableOpacity onPress={() => deleteRow(index)} style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
                            <Text style={{ color: "red", fontSize: 16 }}>✖</Text>
                          </TouchableOpacity>
                        </View>
                      ))}
                      {/* TOTALS ROW */}
                      <View style={[styles.tableRow, { marginTop: 10, borderTopWidth: 1, borderColor: '#ddd', paddingTop: 10, flexDirection: 'row', justifyContent: 'space-between' }]}>
                        <Text style={{ fontWeight: 'bold', fontSize: 14, color: '#000' }}>
                          Total Wt: {items.reduce((sum, it) => sum + (parseFloat(it.weight) || 0), 0).toFixed(3)} g
                        </Text>
                        <Text style={{ fontWeight: 'bold', fontSize: 14, color: '#B8860B' }}>
                          Issue Total: ₹{items.reduce((sum, it) => sum + (it.cost || (parseFloat(it.weight || 0) * parseFloat(it.goldRate || rate || 0))), 0).toFixed(2)}
                        </Text>
                      </View>
                    </View>
                  </ScrollView>
              ...
              <View style={[styles.card, { marginTop: 10 }]}>
                <TouchableOpacity
                  ...
                  onPress={() => {
                    ...
                    if (gstEnabled) { setSgst(""); setCgst(""); setIsSgstEnabled(false); setIsCgstEnabled(false); }
                  }}
                >
                ...
                {gstEnabled && (
                  <View style={{ marginTop: 12 }}>
                    <View style={styles.row}>
                      <View style={styles.inputBox}>
                        ...
                        <TextInput
                          style={styles.input}
                          value={sgst}
                          onChangeText={(v) => { setSgst(v); setIsSgstEnabled((Number(v) || 0) > 0); }}
                          keyboardType="decimal-pad"
                          placeholder="0.00"
                          placeholderTextColor="#9E9E9E"
                        />
                      ...
                      <View style={styles.inputBox}>
                        ...
                        <TextInput
                          style={styles.input}
                          value={cgst}
                          onChangeText={(v) => { setCgst(v); setIsCgstEnabled((Number(v) || 0) > 0); }}
                          keyboardType="decimal-pad"
                          placeholder="0.00"
                          placeholderTextColor="#9E9E9E"
                        />
                    ...
                    {(() => {
                      const issTotal = items.reduce((s, it) => s + (it.cost || (parseFloat(it.weight || 0) * parseFloat(it.goldRate || rate || 0))), 0);
              ...
              <View style={[styles.card, { marginTop: 20 }]}>
                ...
                <View style={styles.row}>
                  <View style={styles.inputBox}>
                    ...
                    <View style={[styles.purityBox, { backgroundColor: '#E8F5E9', padding: 10, borderRadius: 6 }]}>
                      <Text style={{ color: '#2E7D32', fontWeight: 'bold' }}>
                        {fmt(Number(((parseNum(receiptWeight) - parseNum(receiptLess)) * (parseNum(receiptTakenTouch) / 100)).toFixed(3)))} g
                      </Text>
                    </View>
                  </View>
                ...
                <View style={styles.row}>
                  <View style={styles.inputBox}>
                    ...
                    <TextInput
                      style={[styles.input, { borderColor: '#B8860B', borderWidth: 2 }]}
                      value={receiptAmount}
                      onChangeText={setReceiptAmount}
                      keyboardType="decimal-pad"
                      placeholder={
                        (
                          Number(((parseNum(receiptWeight) - parseNum(receiptLess)) * (parseNum(receiptTakenTouch) / 100)).toFixed(3)) *
                          parseNum(receiptGoldRate)
                        ).toFixed(2)
                      }
                      placeholderTextColor="#9E9E9E"
                    />
                ...
                <TouchableOpacity style={[styles.addRowBtn, { marginTop: 20 }]} onPress={addB2CReceiptItem}>
                  <Text style={styles.addRowText}>RECEIPT</Text>
                </TouchableOpacity>
                {b2cReceiptItems.length > 0 && (
                  <View style={{ marginTop: 20 }}>
                    <ScrollView horizontal showsHorizontalScrollIndicator={true}>
                      <View style={{ gap: 5 }}>
                        <View style={[styles.tableHeader, { backgroundColor: '#eee', padding: 10, borderRadius: 6, flexDirection: 'row' }]}>
                          {["Bill No", "Type", "Weight", "Less", "Purity", "Amount (₹)", "X"].map((h, i) => (
                            <Text key={i} style={[styles.th, { flex: 1, textAlign: "center" }]}>{h}</Text>
                          ))}
                        </View>
                        {b2cReceiptItems.map((item, index) => (
                          <View key={index} style={[styles.tableRow, { flexDirection: 'row', padding: 10, borderBottomWidth: 1, borderColor: '#eee' }]}>
                            <Text style={[styles.td, { flex: 1, textAlign: "center" }]}>{item.billNo || "Auto"}</Text>
                            <Text style={[styles.td, { flex: 1, textAlign: "center" }]}>{item.type || item.name}</Text>
                            <Text style={[styles.td, { flex: 1, textAlign: "center" }]}>{fmt(item.weight)}</Text>
                            <Text style={[styles.td, { flex: 1, textAlign: "center" }]}>{fmt(item.less || item.sub || 0)}</Text>
                            <Text style={[styles.td, { flex: 1, textAlign: "center" }]}>{fmt(item.purity || item.netWeight)}</Text>
                            <Text style={[styles.td, { flex: 1, textAlign: "center", color: '#0aa76a', fontWeight: 'bold' }]}>{Number(item.amount || 0).toFixed(2)}</Text>
                            <TouchableOpacity onPress={() => deleteB2CReceiptItem(index)} style={{ flex: 1, alignItems: 'center' }}>
                    ...
                    <View style={[styles.tableRow, { marginTop: 10, borderTopWidth: 1, borderColor: "#ddd", paddingTop: 10 }]}>
                      <Text style={{ flex: 1, textAlign: "right", fontWeight: "bold", fontSize: 14, color: '#0aa76a' }}>
                        Receipt Total: ₹{b2cReceiptItems.reduce((acc, item) => acc + parseFloat(item.amount || 0), 0).toFixed(2)}
                      </Text>
                    </View>
                  </View>
                )}
              ...
              <View style={[styles.card, { marginTop: 20 }]}>
                <View style={styles.issueHeader}>
                  ...
                  <View style={styles.cartContainer}>
                    <Icon name="cash" size={22} color="#FFD700" />
                      ...
                      {["Cash", "GPay", "Card", "Debt"].map((mode) => (
                        <TouchableOpacity
                          ...
                          style={{
                            ...
                            borderRadius: 6,
                            backgroundColor: cashPaymentMode === mode ? '#B8860B' : '#E0E0E0',
                          }}
                          onPress={() => setCashPaymentMode(mode)}
                        >
                          <Text style={{ color: cashPaymentMode === mode ? '#fff' : '#333', fontSize: 12, fontWeight: 'bold' }}>
                            {mode}
                          </Text>
                        </TouchableOpacity>
                      ))}
                ...
                {cashTable.length > 0 && (
                  <View style={{ marginTop: 20 }}>
                    <View style={[styles.tableHeader, { flexDirection: 'row', backgroundColor: '#eee', padding: 10, borderRadius: 6 }]}>
                      <Text style={[styles.th, { flex: 2, textAlign: 'center' }]}>Amount (₹)</Text>
                      <Text style={[styles.th, { flex: 2, textAlign: 'center' }]}>Payment Type</Text>
                      <Text style={[styles.th, { flex: 1, textAlign: 'center' }]}>X</Text>
                    </View>
                    {cashTable.map((row) => (
                      <View key={row.id} style={[styles.tableRow, { flexDirection: 'row', padding: 10, borderBottomWidth: 1, borderColor: '#eee' }]}>
                        <Text style={[styles.td, { flex: 2, textAlign: 'center', fontWeight: 'bold' }]}>₹{Number(row.rupees || 0).toFixed(2)}</Text>
                        <Text style={[styles.td, { flex: 2, textAlign: 'center' }]}>{row.paymentMode || "Cash"}</Text>
                        <TouchableOpacity onPress={() => removeCashEntry(row.id)} style={{ flex: 1, alignItems: 'center' }}>
                          <Text style={[styles.td, { color: "red" }]}>✖</Text>
                        </TouchableOpacity>
                      </View>
                    ))}
                    <View style={{ marginTop: 8, borderTopWidth: 1, borderColor: "#ddd", paddingTop: 8 }}>
                      <Text style={{ textAlign: "right", fontWeight: "bold", fontSize: 14 }}>
                        Cash Total: ₹{cashTable.reduce((sum, c) => sum + Number(c.rupees || 0), 0).toFixed(2)}
                      </Text>
                    </View>
                  </View>
                )}
              ...
              {/* ---------------- BALANCE SUMMARY ---------------- */}
              {(items.length > 0 || b2cReceiptItems.length > 0 || cashTable.length > 0) && (() => {
                const issueTotal = items.reduce((sum, it) => sum + (it.cost || (parseFloat(it.weight || 0) * parseFloat(it.goldRate || rate || 0))), 0);
                const sgstAmt = gstEnabled ? ((parseFloat(sgst) || 0) / 100) * issueTotal : 0;
                const cgstAmt = gstEnabled ? ((parseFloat(cgst) || 0) / 100) * issueTotal : 0;
                const gstTotal = sgstAmt + cgstAmt;
                const receiptTotal = b2cReceiptItems.reduce((sum, it) => sum + parseFloat(it.amount || 0), 0);
                const cashTotal = cashTable.reduce((sum, c) => sum + Number(c.rupees || 0), 0);
                const remainingBalance = issueTotal + gstTotal - receiptTotal - cashTotal;
                return (
                  <View style={{ marginTop: 16, backgroundColor: '#FFF8E1', borderRadius: 8, padding: 14, borderWidth: 1, borderColor: '#F9A825' }}>
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 6 }}>
                      ...
                      <Text style={{ fontWeight: 'bold' }}>₹{issueTotal.toFixed(2)}</Text>
                    </View>
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 6 }}>
                      ...
                      <Text style={{ fontWeight: 'bold', color: '#0aa76a' }}>- ₹{receiptTotal.toFixed(2)}</Text>
                    </View>
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 10 }}>
                      <Text style={{ color: '#333' }}>Cash Received</Text>
                      <Text style={{ fontWeight: 'bold', color: '#1565C0' }}>- ₹{cashTotal.toFixed(2)}</Text>
                    </View>
                    <View style={{ borderTopWidth: 1, borderColor: '#F9A825', paddingTop: 10, flexDirection: 'row', justifyContent: 'space-between' }}>
                      ...
                      <Text style={{ fontSize: 16, fontWeight: 'bold', color: remainingBalance > 0 ? '#D32F2F' : '#2E7D32' }}>
      ...
      {/* BARCODE SCANNER MODAL – B2C Add Item */}
      <Modal
        visible={b2cScannerVisible}
        animationType="slide"
        onRequestClose={() => setB2cScannerVisible(false)}
      >
        <View style={b2cLocalStyles.scannerContainer}>
          <CameraView
            ...
            onBarcodeScanned={({ data }) => {
              setB2cScannerVisible(false);
              fetchStockByBarcodeB2C(data);
            }}
          />
          <View style={b2cLocalStyles.scannerOverlay}>
            <View style={b2cLocalStyles.scannerFrame} />
            <Text style={b2cLocalStyles.scannerHint}>Point camera at barcode</Text>
            <TouchableOpacity
              style={b2cLocalStyles.scannerClose}
              onPress={() => setB2cScannerVisible(false)}
            >
              <Text style={b2cLocalStyles.scannerCloseText}>Cancel</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
...
const b2cLocalStyles = StyleSheet.create({
  barcodeRow: {
    flexDirection: "row",
    alignItems: "center",
    gap: 8,
