...
const EntryForm = ({
  ...
  onScan, barcode, setBarcode, onBarcodeSubmit
}) => {
        ...
        <View style={styles.inputGroup}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
            <TextInput
              ...
              onSubmitEditing={() => onBarcodeSubmit(barcode)}
            />
            <TouchableOpacity
              style={styles.scanBtn}
              onPress={() => onBarcodeSubmit(barcode)}
            >
...
export default function SuspenseTransactionScreen({ navigation, route }) {
  ...
  const [scannerFor, setScannerFor] = useState('issue');
    ...
    useCallback(() => {
      const loadData = async () => {
        try {
          ...
          const mappedStock = safeStockData.map(it => ({
            ...it,
            stockName: it.itemName, // Match legacy key
            sellingTouch: it.buyingTouch || 100 // Fallback
          }));
          setAllStockItems(mappedStock);
        } catch (error) {
  ...
  const handleBarcodeSubmit = (code) => {
    if (!code?.trim()) return;
    const found = allStockItems.find(item => item.barcode === code.trim());
    if (found) {
      selectStockItem(found, 'issue');
      setIssueWeight(String(found.netWeight || found.grossWeight || ''));
      setIssueBarcode('');
    } else {
      Alert.alert("Not Found", `No stock item found for barcode: ${code}`);
    }
  };
  ...
  const openScanner = (type) => {
    ...
    if (!permission?.granted) {
      requestPermission().then(({ granted }) => {
        ...
        else Alert.alert("Permission Denied", "Camera permission is required to scan barcodes.");
      });
    } else {
      setShowScanner(true);
    }
  ...
  const handleBarcodeScan = ({ data }) => {
    ...
    const found = allStockItems.find(item => item.barcode === data);
    if (found) {
      selectStockItem(found, scannerFor);
      if (scannerFor === 'issue') {
        setIssueWeight(String(found.netWeight || found.grossWeight || ''));
        setIssueCount('1');
      } else {
        setReceiptWeight(String(found.netWeight || found.grossWeight || ''));
        setReceiptCount('1');
      }
    } else {
      Alert.alert("Not Found", `No stock item found for barcode:\n${data}`);
    }
  };
          ...
          <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
            <View style={{ flex: 1, paddingBottom: 40 }}>
              ...
              <View style={styles.modernCard}>
                ...
                <View style={styles.formHeader}>
                  <Ionicons name="person-circle" size={24} color="#3D2800" />
              ...
              <EntryForm
                ...
                setBarcode={setIssueBarcode}
                onBarcodeSubmit={handleBarcodeSubmit}
              />
              <DataTable items={issueItems} type="Issue" color="#D32F2F" removeItem={removeItem} />

              <EntryForm
                title="Receipt" name={receiptName} setName={setReceiptName} w={receiptWeight} setW={setReceiptWeight} c={receiptCount} setC={setReceiptCount} p={receiptPure} setP={setReceiptPure} r={receiptRate} setR={setReceiptRate} onAdd={addReceiptItem} color="#B8860B"
                handleItemSearch={handleItemSearch}
                showItemDropdown={showItemDropdown}
                issueFilteredStocks={issueFilteredStocks}
                receiptFilteredStocks={receiptFilteredStocks}
                selectStockItem={selectStockItem}
              />
              <DataTable items={receiptItems} type="Receipt" color="#B8860B" removeItem={removeItem} />

              {/* SUMMARY */}
              <View style={styles.totalCard}>
                <View style={styles.summaryRow}>
                  <Text style={styles.totalLabel}>Net Pure Gold:</Text>
                  <Text style={[styles.totalValue, { color: netPure >= 0 ? '#D32F2F' : '#B8860B' }]}>{netPure.toFixed(3)} g</Text>
                </View>
                <View style={styles.summaryRow}>
        ...
        <View style={{ flex: 1, backgroundColor: '#000' }}>
          {permission?.granted ? (
            ...
            >
              <View style={styles.scannerOverlay}>
                <Text style={styles.scannerTitle}>
                  Scan {scannerFor === 'issue' ? 'Issue' : 'Receipt'} Item Barcode
                </Text>
            ...
            <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', padding: 30 }}>
              <Ionicons name="camera-outline" size={60} color="#fff" style={{ marginBottom: 20 }} />
              <Text style={{ color: '#fff', fontSize: 16, textAlign: 'center', marginBottom: 20 }}>
                Camera permission is required to scan barcodes.
              </Text>
              <TouchableOpacity
                style={{ padding: 14, backgroundColor: '#D32F2F', borderRadius: 10 }}
                onPress={requestPermission}
              >
                <Text style={{ color: '#fff', fontWeight: '700' }}>Grant Camera Permission</Text>
              </TouchableOpacity>
              <TouchableOpacity style={{ marginTop: 20 }} onPress={() => setShowScanner(false)}>
...
const styles = StyleSheet.create({
  ...
  saveText: { fontWeight: "800", color: "#3D2800" },
