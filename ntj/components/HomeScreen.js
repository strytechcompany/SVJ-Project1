...
export default function HomeScreen({ route }) {
  ...
  const loadRates = useCallback(async () => {
    try {
      ...
      if (response.ok) {
        ...
        if (data.goldDate) setGoldDate(data.goldDate);
        return;
      } else {
  ...
  const handleSave = async () => {
    const payload = { goldRate, goldDate };
    ...
    try {
      ...
      if (response.ok) {
        ...
        Alert.alert("Success", "Gold rate saved successfully!");
      } else {
    ...
    <View style={{ flex: 1, backgroundColor: "#FFF8E1" }}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.goldCard}>
          ...
          <View style={[styles.cardRow, { justifyContent: "center" }]}>
            <View style={{ alignItems: "center" }}>
              ...
              <Text style={[styles.goldLabel, { marginTop: 4, fontSize: 12 }]}>{goldDate}</Text>
            </View>
          </View>
        ...
        <View style={styles.quickGrid}>
          {[
            { label: "B2B", icon: "handshake-outline", color: "#7A5C00", bg: "#FFF3CD", nav: () => navigation.navigate("B2BCalculationPage", { goldRate }), badge: quickAccessCounts.b2b },
      ...
      <View style={styles.bottomNav}>
        ...
        <TouchableOpacity onPress={() => navigation.navigate("StockMaster")} style={styles.bottomNavItem}>
          <FontAwesome name="money" color="#FFE082" size={24} />
          <Text style={styles.bottomNavLabel}>Stock Master</Text>
        </TouchableOpacity>
      ...
      <Modal visible={modalVisible} transparent animationType="slide">
        <View style={styles.modalOverlay}>
          <View style={styles.modalBox}>
            <Text style={styles.modalTitle}>Edit Gold Rate & Date</Text>

            <Text style={styles.modalLabel}>Gold Rate (₹ per gram)</Text>
            <TextInput
              style={styles.input}
              value={goldRate}
